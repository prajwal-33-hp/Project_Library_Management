...
# ── Database Configuration (mirrors app.py) ──────────────────────────────────
DB_CONFIG = {
...
def get_connection(with_db=True):
    """Return a MySQL connection, optionally without selecting a database."""
    cfg = DB_CONFIG.copy()
    if not with_db:
        cfg.pop('database')
...
def ensure_database():
    ...
    raise RuntimeError(
        ...
        "Check your MYSQL_* environment variables."
    )
...
def insert_books():
    """Insert all sample books, skipping any that already exist."""
    conn = get_connection()
    cur = conn.cursor()
    inserted = 0
    skipped = 0
    ...
    for title, author, quantity in BOOKS:
        # Skip if a book with the same title and author already exists
        cur.execute(
    ...
    print("\nLog in as Staff and browse to /staff/books to see the full collection.")


if __name__ == '__main__':
    print("📚  Populating LibraVault database with 500 books...\n")
    ensure_database()
    ensure_books_table()
    insert_books()
